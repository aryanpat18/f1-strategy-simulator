...
# ---------------------------------------------------------------------------
# Ensure project root is importable (works at repo root and inside Docker)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Quick API connectivity check (fail fast instead of hanging)
# ---------------------------------------------------------------------------
@st.cache_data(ttl=60, show_spinner=False)
def _check_api(url: str) -> bool:
    try:
        requests.get(f"{url}/data/races", params={"year": 2024}, timeout=5)
        return True
    except Exception:
        return False
